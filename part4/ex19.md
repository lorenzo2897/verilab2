...
We also implemented a variable delay echo synthesiser, where the time between echoes is determined by the switches set by the user.

The key is to implement our own version of a FIFO, so that we can tap into the buffer at any location we wish, instead of being constrained by only reading the value at the end of the line.
